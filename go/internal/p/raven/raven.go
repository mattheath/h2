...
var (
	...
	AmqpUri = flag.String("rabbituri", "", "example: amqp://guest:guest@rabbitmq:5672")
)
...
func init() {
	flag.Parse()
	if envAMQP := os.Getenv("BOXEN_RABBITMQ_URL"); *AmqpUri == "" && envAMQP != "" {
		AmqpUri = &envAMQP
	}
	if *AmqpUri != "" {
		if config, err := amqp.ParseURI(*AmqpUri); err == nil {
			fmt.Printf("Using custom rabbit config: %v", AmqpUri)
		...
		t := fmt.Sprintf("amqp://%s:%s@%s:%s/", USERNAME, PASSWORD, HOSTNAME, strconv.Itoa(*PORT))
		AmqpUri = &t
	}
